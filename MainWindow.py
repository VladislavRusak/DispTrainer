...
class MainFormWindow(QWidget):
    ...
    def setMainUi(self):
        ...
        thead = ["ID", "№ состава", "Экспедитор", "дата и время подхода состава", "Вид груза", "Количество вагонов",
                 "Только прямой вариант (вагон - судно)", "Путь"]
        col_num = 0
        for val in thead:
    ...
    def addMeteoDo(self):
        ...
        if not error:
            ...
            meteo = Meteo(float(self.windV), int(self.windStart.dateTime().toTime_t()),
                          int(self.windEnd.dateTime().toTime_t()), self.precipitation)
            METEO.append(meteo)
            self.wid.close()
        else:
            QMessageBox.about(self, 'Ошибка!', error_text)
    ...
    def delMeteo(self, val):
        ...
        if val == 1:
            ...
            for x in range(1, self.th_table.rowCount()):
                meteo = Meteo(self.th_table.item(x, 0).text(), self.th_table.item(x, 1).text(),
                              self.th_table.item(x, 2).text())
                METEO.append(meteo)
            print(METEO)
...
class FiniteForm(QWidget):
    ...
    def ShowMainLog(self):
        ...
        thead = ["Время начала", "Время окончания", "тип объекта", "объект", "Причал/путь", "Тип второго объекта",
                 "Второй объект", "Причал/путь", "Тип груза", "Количество груза", "краны", "количество людей",
                 "Количество техники"]
        col_num = 0
        for val in thead:
